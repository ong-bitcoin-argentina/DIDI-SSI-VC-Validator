...
const valid = { 
        "iat": 1626782449,
        "sub": "did:ethr:0x36e826948ac29f18b3c8c9ae96eeab82f168e540",
        "vc": {
          "@context": [
          ...
          "credentialSubject": {
            "Semillas Vivienda": {
              ...
              "data": {
                ...
                "Distrito de Residencia": "malvinas argentinas",
                "Direccion": "estados unidos 2339",
                "Barrio": "primaveral",
                "Localidad": "grandbourg",
                "Condiciones grales": "Sin terminar",
                "Red de gas": "true",
                "Garrafa": "false",
                "Red de agua": "false",
                "Pozo/Bomba": "true",
...
test(`Validate .vc.credentialSubject['Semillas Vivienda'].preview.type field FAIL`, async() =>{
    const result = await validateCredential(livingPlace.v1, invalidPreviewTypeJWT);
    expect(result.status).toBe(false);
    expect(result.errors[0].keyword).toBe('type');
    expect(result.errors[0].dataPath).toBe(`.vc.credentialSubject['Semillas Vivienda'].preview.type`);
    expect(result.errors[0].schemaPath).toBe('#/properties/vc/properties/credentialSubject/properties/Semillas%20Vivienda/properties/preview/properties/type/type');
...
test(`Validate .vc.credentialSubject['Semillas Vivienda'].data.type field FAIL`, async() =>{
    ...
    expect(result.status).toBe(false);
    expect(result.errors[0].keyword).toBe('type');
    expect(result.errors[0].dataPath).toBe(`.vc.credentialSubject['Semillas Vivienda'].data['CERTIFICADO O CURSO']`);
    expect(result.errors[0].schemaPath).toBe('#/properties/vc/properties/credentialSubject/properties/Semillas%20Vivienda/properties/data/properties/CERTIFICADO%20O%20CURSO/type');
    expect(result.errors[0].params.type).toBe('string');
    expect(result.errors[0].message).toBe('should be string');
  });
...
test(`Validate .vc.credentialSubject.['Semillas Vivienda'].category.type field FAIL`, async() =>{
    ...
    expect(result.status).toBe(false);
    expect(result.errors[0].keyword).toBe('type');
    expect(result.errors[0].dataPath).toBe(`.vc.credentialSubject['Semillas Vivienda'].category`);
    expect(result.errors[0].schemaPath).toBe('#/properties/vc/properties/credentialSubject/properties/Semillas%20Vivienda/properties/category/type');
    expect(result.errors[0].params.type).toBe('string');
    expect(result.errors[0].message).toBe('should be string');
  });