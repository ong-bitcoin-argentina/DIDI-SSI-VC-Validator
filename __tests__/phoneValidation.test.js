...
test('Validate iat field FAIL', async () => {
    result = await validateCredential(phoneSchema, phoneIatFieldFail);
...
test('Validate sub field FAIL', async () => {
    result = await validateCredential(phoneSchema, phoneSubFieldFail);
    expect(result.status).toBe(false);
    expect(result.errors[0].keyword).toBe('type');
...
test('Validate vc.credentialSubject.Phone.preview.type field FAIL', async () => {
    result = await validateCredential(phoneSchema, phoneJwtTypeFieldFail);
    expect(result.status).toBe(false);
    expect(result.errors[0].keyword).toBe('type');
...
test('Validate iss field FAIL', async () => {
    result = await validateCredential(phoneSchema, phoneIssFieldFail);
    expect(result.status).toBe(false);
    expect(result.errors[0].keyword).toBe('type');
