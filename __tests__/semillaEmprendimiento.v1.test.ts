...
test('Validate sub field FAIL', async() => {
    const result = await validateCredential(semillaEmprendimiento.v1, invalidSubJWT);
...
test('Validate iss field FAIL', async () => {
    const result = await validateCredential(semillaEmprendimiento.v1, invalidIssJWT);
    expect(result.status).toBe(false);
    expect(result.errors[0].keyword).toBe('type');
...
test(`Validate .vc.credentialSubject.Emprendimiento.preview.type field FAIL`, async() =>{
    const result = await validateCredential(semillaEmprendimiento.v1, invalidPreviewTypeJWT);
    expect(result.status).toBe(false);
    expect(result.errors[0].keyword).toBe('type');
    expect(result.errors[0].dataPath).toBe(`.vc.credentialSubject.Emprendimiento.preview.type`);
    expect(result.errors[0].schemaPath).toBe('#/properties/vc/properties/credentialSubject/properties/Emprendimiento/properties/preview/properties/type/type');
...
test(`Validate .vc.credentialSubject.Emprendimiento.data.type field FAIL`, async() =>{
    const result = await validateCredential(semillaEmprendimiento.v1, invalidDataTypeJWT);
    expect(result.status).toBe(false);
    expect(result.errors[0].keyword).toBe('type');
    expect(result.errors[0].dataPath).toBe(`.vc.credentialSubject.Emprendimiento.data.credentialName`);
    expect(result.errors[0].schemaPath).toBe('#/properties/vc/properties/credentialSubject/properties/Emprendimiento/properties/data/properties/credentialName/type');
    expect(result.errors[0].params.type).toBe('string');
    expect(result.errors[0].message).toBe('should be string');
  });
...
test(`Validate .vc.credentialSubject.Emprendimiento.category.type field FAIL`, async() =>{
    const result = await validateCredential(semillaEmprendimiento.v1, invalidCategoryJWT);
    expect(result.status).toBe(false);
    expect(result.errors[0].keyword).toBe('type');
    expect(result.errors[0].dataPath).toBe(`.vc.credentialSubject.Emprendimiento.category`);
    expect(result.errors[0].schemaPath).toBe('#/properties/vc/properties/credentialSubject/properties/Emprendimiento/properties/category/type');
    expect(result.errors[0].params.type).toBe('string');
    expect(result.errors[0].message).toBe('should be string');
  });