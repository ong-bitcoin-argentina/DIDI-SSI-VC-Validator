...
const { legalAddressSchema } = require('../src/schema/address/legalAddress-schema-v1');
...
test('Validate OK', async () => {
    result = await validateCredential(legalAddressSchema, validJwt);
    expect(result.status).toBe(true);
    expect(result.errors).toBe(null);
});

test('Validate iat field FAIL', async () => {
  result = await validateCredential(legalAddressSchema, invalidIat);
...
test('Validate sub field FAIL', async() => {
  result = await validateCredential(legalAddressSchema, invalidSub);
  expect(result.status).toBe(false);
  expect(result.errors[0].keyword).toBe('type');
